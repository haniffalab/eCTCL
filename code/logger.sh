...
function logger() {
    ...
    while [[ "${1}" =~ ^- ]] || [[ "${1}" =~ ^INFO ]]; do shift; done
    [ -z "${4}" ] && LTYPE="\033[1;32mINFO\033[0m" || LTYPE="${4}"
    PNAME="$(git rev-parse --show-toplevel 2>&1)"
    [ ! -z "(echo ${PNAME} | grep -q fatal)" ] && PNAME="$(basename $(pwd))"
    LOGLINE="[$(date '+%Y-%m-%d %T')] ${LTYPE} ${PNAME} "
...
function logger_info () {
    logger "${1}" "${2:-80}" "${3:-#}" "\033[1;32mINFO\033[0m"
}
function logger_warn () {
    logger "${1}" "${2:-80}" "${3:-#}" "\033[1;33mWARN\033[0m"
}
function logger_error () {
    logger "${1}" "${2:-80}" "${3:-#}" "\033[1;31mERROR\033[0m"
}
function logger_debug () {
    logger "${1}" "${2:-80}" "${3:-#}" "\033[1;34mDEBUG\033[0m"
}
